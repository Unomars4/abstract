...
type ChartContext<T> = Record<string, ChartInfo<T>>;

export const useChartsStore = <T>() =>
  defineStore('charts', () => {
    const chartContexts = ref<ChartContext<T>>({});
    ...
    async function initChartCtx({
      ...
      try {
        ...
        const data = await d3.json<T[]>(url);
        if (data) {
    ...
    function getChartData(chartId: string): T[] | [] {
